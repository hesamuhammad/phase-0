 ...
 * array of obiect. Array of Object ini merepresentasikan penumpang yang naik ke dalam kereta.
...
function trainSeating(penumpang) {
    ...
    for (let i = 0; i < penumpang.length; i++) {
      for (let j = 0; j < destinations.length; j++) {
        if (destinations[j] === penumpang[i]['awal']) {
          naik = j
          // console.log(naik)
        } else if (penumpang[i]['akhir'] === destinations[j]) {
          akhir = j
          // console.log(akhir)
        }
        var hasilHitung = akhir - naik
      }
      if (obj[hasilHitung] === undefined) {
        obj[hasilHitung] = []
      }
      // console.log(obj)
      for (const key in obj) {
        if (key == hasilHitung) {
          obj[hasilHitung].push({
